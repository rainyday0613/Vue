...
public class VueBoardRepository {
    ...
    public void count(Long count) throws Exception {
        String query = "update cafe_board set count = count+1 where board_no = ?";
        jdbcTemplate.update(query, count);
    }

    public List<VueBoard> list() throws Exception {
        List<VueBoard> results = jdbcTemplate.query(
                "select board_no, title, content, writer, reg_date, count from cafe_board " +
